...
------------------------------------------------------------------ */
function PhotoCard({ photo, distance, isCenter, dims, positionX, aspect, onAspect, tweaks }) {
  ...
  // Tap detection lives in the stage handler (below) because the stage
  // captures the pointer for dragging. Stamp the photo id + index on the
  // card so `elementFromPoint` can identify it.

  return (
    <div
      data-photo-id={photo.id}
      data-photo-center={isCenter ? "1" : "0"}
      style={{
        position: "absolute",
        left: "50%",
        top: "50%",
        ...
        cursor: isCenter && (photo.img || photo.placeholder) ? "zoom-in" : undefined
      }}>
...
function App() {
  ...
  // Refs so the pointer handlers (set up once) see the latest values.
  const indexRef = useRef(index);
  indexRef.current = index;
  const photosRef = useRef(photos);
  photosRef.current = photos;
  const openLightboxRef = useRef(null);
  openLightboxRef.current = (id) => setLightboxId(id);

  useEffect(() => {
    const el = stageRef.current;
    if (!el) return;
    let dragStartX = 0,dragStartY = 0,dragStartT = 0,dragging = false;
    let downTarget = null,downTime = 0;
    const onDown = (e) => {
    ...
    const onUp = (e) => {
      ...
      // Tap-vs-drag: small movement + quick release = treat as a click.
      const moved = Math.hypot(
        (e.clientX ?? dragStartX) - dragStartX,
        (e.clientY ?? dragStartY) - dragStartY
      );
      const quick = Date.now() - downTime < 500;
      if (moved < 6 && quick && downTarget) {
        // Walk up from the down target to find a photo card.
        const card = downTarget.closest && downTarget.closest("[data-photo-id]");
        if (card) {
          ...
          if (i >= 0) {
            if (i === indexRef.current) {
              const p = list[i];
              if (p && p.img && openLightboxRef.current) {
